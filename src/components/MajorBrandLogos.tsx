...
interface MajorBrandLogosProps {
  className?: string
  company?: 'Adobe' | 'Amazon' | 'Apple' | 'Bitcoin' | 'Discord' | 'Dropbox' | 'Facebook' | 'Figma' | 'Framer' | 'GitHub' | 'Google' | 'Instagram' | 'LinkedIn' | 'Microsoft' | 'Netflix' | 'Notion' | 'Paypal' | 'Pinterest' | 'Slack' | 'Spotify' | 'Stripe' | 'Telegram' | 'Twitch' | 'X-Twitter' | 'Webflow' | 'WhatsApp' | 'Wise' | 'YouTube'
  style?: 'Original' | 'Black' | 'White'
}

export const MajorBrandLogos: React.FC<MajorBrandLogosProps> = ({ 
  className = '', 
  company = 'Google', 
  style = 'Original' 
}) => {
  if (company === 'Google' && style === 'Original') {
    return (
  ...
  // Default Adobe logo (fallback)
  return (
    <div className={`relative w-5 h-5 ${className}`}>
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M10 0L0 20H6.67L10 12.5L13.33 20H20L10 0Z" fill="#FF0000"/>
      </svg>
    </div>
  )
}
